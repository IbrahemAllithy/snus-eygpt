...
class OrderService
{
    ...
    public function orderEmail($order)
    {
        ...
        $search = ['[[status]]', '[[order_id]]', '[[buyer_name]]', '[[shipping_address]]', '[[amount]]'];

        $replace = [$order->order_status, $order->id, $order->billing_first_name, $order->delivery_street_aadress, $order->order_price];

        $order_email_template_with_data = str_replace($search, $replace, $order_email_template);

        $table_row = '';
        for ($i = 0; $i < count($product_discount) && $i < count($total) && $i < count($product_name) && $i < count($product_price) && $i < count($product_image) && $i < count($quantity); $i++) {

            $table_row .= '<tr style="border-bottom: 1px solid rgba(0,0,0,.05);"><td valign="middle" width="80%" style="text-align:left; padding: 0 2.5em;"><div class="product-entry"><img src="'.url('/gallary/'.$product_image[$i]).'" alt="" style="width: 100px; max-width: 600px; height: auto; margin-bottom: 20px; display: block;"><div class="text"><h3>'.$product_name[$i].'</h3><p>Qty:'.$quantity[$i].'</p><p>Price:'.$product_price[$i].'</p><p>Discount Price:'.$product_discount[$i].'</p></div></div></td><td valign="middle" width="20%" style="text-align:left; padding: 0 2.5em;"><span class="price" style="color: #000; font-size: 20px;">'.number_format($total[$i] * $quantity[$i], 2).'</span></td></tr>';
        }
