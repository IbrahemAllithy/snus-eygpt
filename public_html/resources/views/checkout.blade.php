        ...
        <tbody>
            <tr class="d-flex cartItem-row">
                <td class="col-12 col-md-2">
                    <img class="img-fluid cartItem-image" src="{{ asset('assets/images/snuslogo1.png') }}" alt="Cart product" />
                </td>
...
@section('script')
                        ...
                        for (i = 0; i < data.data.length; i++) {
                            ...
                            if (data.data[i].product_type == 'variable') {
                                ...
                                total_weight += parseFloat(data.data[i].product_weight) * parseFloat(data.data[i]
                                    .qty);
                                var checkoutGallery = data.data[i].product_gallary && Array.isArray(data.data[i].product_gallary.detail)
                                    ? data.data[i].product_gallary.detail : [];
                                var checkoutImage = checkoutGallery[2] || checkoutGallery[1] || checkoutGallery[0];
                                if (checkoutImage && checkoutImage.gallary_path) {
                                    clone.querySelector(".cartItem-image").setAttribute('src', checkoutImage.gallary_path);
                                }
    ...
    @if (trim((string) (getSetting()['google_map_api_string'] ?? '')) !== '')
        <script
            src="https://maps.googleapis.com/maps/api/js?key={{ getSetting()['google_map_api_string'] }}&libraries=places&callback=initialize"
            async defer></script>
    @endif
    <script>
        var markers;
